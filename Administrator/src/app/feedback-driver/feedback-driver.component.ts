...
@Component({
  selector: 'app-feedback-driver',
  templateUrl: './feedback-driver.component.html',
  styleUrls: ['./feedback-driver.component.scss']
})
export class FeedbackDriverComponent implements OnInit {
  items: any;
  ...
  ngOnInit() {
    this.ph._getFeedDriverProfile().on('value', snapshot => {
      this.items = [];
    //  this.pop.hideLoader()
 ...
 openDialog3(g): void {
  ...
  Delete(item){
    this.ph.getFeedDriverProfile(item).remove().then(f =>{
